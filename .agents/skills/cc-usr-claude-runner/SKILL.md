...
By default, `--mode auto` uses headless execution. On macOS with iTerm2 installed, it opens a visible iTerm2 window and passes `--headless`; otherwise it uses a detached process with no terminal or stdin. Headless mode uses Claude Code print mode plus `--permission-prompts none`, so workspace-trust and permission prompts cannot block the run. `--mode server` is the same unattended path; `--mode iterm` is an explicit native interactive override and may show Claude Code's trust prompt. At launch time, the helper re-reads `.env` and loads the exact prompt from SQLite. Interactive launches use `[claude_binary, "--dangerously-skip-permissions", stored_prompt]`; headless launches add `--print`, an explicit bypass permission mode, and disabled permission prompts. Use it only in the isolated question workspace.
