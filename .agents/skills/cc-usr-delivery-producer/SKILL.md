...
5. Build a five-dimension evidence ledger before scoring. Every exact file, function, command, line, count, error and test result must be traceable to an event in the matched JSONL or to the resulting workspace; evidence produced later by QC or another machine cannot be attributed to the target turn.
...
11. After every valid turn is stored, hand the batch to `$cc-usr-delivery-qc`. Do not approve records or export Excel in this skill.
