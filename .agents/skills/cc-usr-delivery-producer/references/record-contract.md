...
`evidence_ledger` is a JSON array that binds every sentence in all five descriptions to a source. Trajectory entries include the exact one-based JSONL line, excerpt and SHA-256; workspace entries include a safe relative path, excerpt and full-file SHA-256; prompt entries include the exact excerpt and prompt SHA-256. A trajectory line must fall after this turn's real user event and before the next real user event. `requirement_coverage` quotes each material requirement exactly, records `met`/`unmet`/`uncertain`, and points to evidence IDs. Missing evidence, a later-turn source, a changed hash, or an unbound sentence blocks insertion.
