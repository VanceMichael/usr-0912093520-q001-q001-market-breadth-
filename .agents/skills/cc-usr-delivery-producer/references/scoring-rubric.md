...
Only model-attributable facts may lower a score. An absent host command or preinstalled dependency, unavailable Docker daemon, network/registry/gateway failure, permission or ownership restriction, certificate failure, and similar infrastructure condition cannot support a deduction in any dimension. Keep the raw event in the trajectory but omit it from exported five-dimension descriptions and `其他问题`. This exclusion does not protect model-authored Dockerfile/Compose defects, unnecessary repeated retries after the external cause is already known, false success claims, or failure to use an available alternative that the prompt required.

Before insertion, compare each score with its own description and then read all five together. After excluding non-model environment failures, if the prose says “没有计划/状态记录”, “多次由模型自身造成的失败后仍有失败”, “模型可执行但未验证”, or equivalent, the corresponding score cannot be 4 or 5. If a 4/5 score is justified, the description must name the observed plan, successful verification, or efficient recovery that earns it. Never use a higher score to compensate for a good result in another dimension.
...
Do not turn those ingredients into visible headings, a checklist, or a repeated sentence frame. Vary the opening, rhythm, and level of detail according to the evidence. Some findings read most naturally from the failed requirement to the code evidence; others should start with a command failure, a mistaken decision, or a specific file. Use “根因是” or “正确做法是” only when it genuinely improves clarity, not as a mandatory tail. Treat this as a blocking quality gate: run `scripts/check_description_style.py` over all pending record JSON files before `collect_record.py`; a failure means the descriptions must be rewritten before insertion.
...
Before storing a turn, read the five descriptions as a set. They must not be identical paraphrases with only the dimension name changed, and they must not start with fixed labels such as `When:`、`What:`、`Impact:`、“过程：”或“产物：”. Openers such as “经检查”“根据轨迹”“综合来看”“本次任务中”“总体而言”“综上所述” expose a writing formula instead of getting to the engineering fact. Do not use arrows or bracketed placeholders. Except for necessary file names, commands, identifiers and exact technical terms, the prose must be Chinese rather than translated English or mixed-language boilerplate. Do not use the same “事实罗列 + 转折结论” frame across a batch, and avoid opening fields with “本轮”“这轮”“这一轮”. The checker is the minimum gate; a successful exit still requires a human reread for natural wording.
