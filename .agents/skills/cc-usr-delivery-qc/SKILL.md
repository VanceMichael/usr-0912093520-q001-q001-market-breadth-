...
2. Review all 28 export fields, not just the reported mechanical errors. Confirm exact prompts, identifiers and trajectory file names, the one-based `当前对话轮次排序`, valid enums and score ranges, snapshot and session consistency, timestamps, parent chains, score-description agreement, and natural evidence-based descriptions. `审核备注` is populated only by finalization. Treat every reported error and warning as an immediate repair task; do not defer it or mark the batch passed while it remains.
   ...
   - A failed test or runtime check does not justify dropping the produced record. Determine whether the cause is attributable to the target model before scoring it. Model-authored code, configuration, decisions, false claims, and avoidable tool misuse may lower a score. Missing host commands or packages, an unavailable Docker daemon, network/registry/gateway failures, permissions, certificates, and similar infrastructure limits must not appear in exported descriptions or lower a score. Only an unrecoverable trajectory, identity, turn-boundary, or contamination failure keeps the record blocked.
   ...
   - Line, step, turn and similar ordinals use Arabic digits such as `第80行`, not Chinese-number forms such as `第八十行`.
   - For new semantic evidence ledgers, verify every item has `kind` and follows the source boundary: requirement/prompt, product/workspace, process/trajectory tool activity, runtime/trajectory tool result. A runtime result must point through `related_line` to the earlier matching tool call in the same turn.
   ...
   - Before accepting any score below 5, confirm every negative sentence describes a model-controllable cause. Remove environment-only deductions rather than disguising them; when no other supported defect remains, raise the score to the matching anchor and rewrite the description from verified positive evidence. Never invent a different weakness merely to preserve the old score.
