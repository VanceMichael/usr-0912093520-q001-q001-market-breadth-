...
- The matched JSONL must be clean for this question. Reject it if any nested event invokes `Skill`/slash-command loading or reads `.agents/skills`, `.claude/skills`, `.codex/skills`, or `SKILL.md` outside the current question workspace. Such contamination cannot be repaired in SQLite; require a new clean model run.
