...
- The matched JSONL contains exactly one byte-for-byte `(SessionID, PromptID, User Prompt)` event. Require a single session, order-sensitive `tool_use.id` to `tool_result.tool_use_id` pairing, a final text-bearing assistant event, and trajectory-derived user-turn order equal to `当前对话轮次排序`.
