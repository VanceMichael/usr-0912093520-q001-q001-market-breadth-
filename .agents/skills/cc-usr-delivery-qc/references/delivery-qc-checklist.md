...
- Every sentence is bound in `evidence_ledger`; source excerpts and hashes still match, trajectory lines fall inside the record's own user-turn boundary, and every material prompt requirement appears in `requirement_coverage`. New ledgers classify evidence as requirement/prompt, product/workspace, process/trajectory, or runtime/trajectory. Runtime evidence cites a real `tool_result` and points to the preceding matching `tool_use` in the same turn. A later turn's result cannot support an earlier record.
...
- All deductions are caused by model-controllable behavior. Host environment, network, missing preinstalled tools or packages, unavailable Docker daemon, registry/gateway, permission/ownership, and certificate failures stay only in the raw trajectory and never appear as an exported scoring reason. Model-authored Dockerfile or Compose defects and avoidable repeated retries remain valid model evidence.
