...
Each prompt is one natural Chinese paragraph whose primary action is to build a complete project or a complete module from zero. It must not describe extending or repairing an already implemented capability. Apply [content-quality.md](content-quality.md) to the prompt and the prepared repository before publication.
