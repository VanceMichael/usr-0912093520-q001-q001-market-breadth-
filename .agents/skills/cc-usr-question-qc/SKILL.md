...
3. Review the prompt pairs semantically for noun-swapped templates, repeated sentence structure, substantially identical business flows, and copied endings that numeric similarity may miss. Build a sentence map for the selected batch: opening perspective, primary action, clause order, acceptance evidence, and final two clauses. A batch of five or more must have at least three distinct opening perspectives and three distinct requirement/acceptance orders. Also reject a standalone checklist-style validation tail even when it does not match another prompt.
   - Reject a standalone `TODO` token even when the prompt uses it negatively. Rewrite the clause as natural Chinese completed-delivery behavior, then rerun prompt-update, duplicate comparison, and mechanical QC.
...
5. Store the duplicate-review result with `qc-set`. When the repaired batch has no duplicate or template finding, use `--decision pass --report '质检通过'` exactly. The command rechecks style, duplicate, and terminal-sentence findings and must refuse `pass` while any remain.
...
- Reject `repeated_terminal_sentence` findings and any checklist-style validation tail or generic environment commentary reported by mechanical QC. Reordering the same clauses or changing a synonym is not a repair.
